...
#define KP_SERVO 1.5  // Aumenta si necesitas más corrección, disminuye si oscila
#define KD_SERVO 0.05 // Añadido para suavizar las correcciones y evitar sobrepasos
...
#define ANGULO_GIRO_IZQUIERDA_PRONUNCIADO -85 // Giro más pronunciado a la izquierda
#define ANGULO_GIRO_DERECHA_PRONUNCIADO   85  // Giro más pronunciado a la derecha
...
// Lee todos los sensores de distancia
void leerSensores() {
...
void manejarEstadoNavegando() {
  ...
  if (detectarColorAzul()) {
    Serial3.println("Linea azul detectada. Decidiendo giro...");
    detectoLineaAzul = true;
    estadoActual = ESTADO_DECIDIR_GIRO;
    return;
  }

  // Prioridad 2: Detección de obstáculo frontal que requiere giro
  if (distanciaFrontal < DISTANCIA_UMBRAL_GIRO_FRONTA) {
    Serial3.println("Obstaculo frontal detectado. Decidiendo giro...");
...
void loop() {
  leerSensores(); // Siempre lee los sensores primero
